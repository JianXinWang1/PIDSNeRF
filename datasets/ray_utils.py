...
def get_ray_directions(H, W, K, device='cpu', random=False, return_uv=False, flatten=True,train=False):
    """
...
def center_poses(poses,point_loss_data, pts3d=None):
    """
    ...
    if pts3d is not None:
        ...
        for i in range(len(point_loss_data)):
            point_loss_data[i]['pts'] = (point_loss_data[i]['pts'] @pose_avg_inv[:, :3].T + pose_avg_inv[:, 3:].T)[:,0:3]
