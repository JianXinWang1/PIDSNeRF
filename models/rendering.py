...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        for i in range(0, len(valid_mask), 327680):
            if ~valid_mask[i:i + 327680].any():
                continue
            sigmas_tem, _rgbs = \
                model(xyzs[i:i + 327680][valid_mask[i:i + 327680]], dirs[i:i + 327680][valid_mask[i:i + 327680]],
                      **kwargs)
            rgbs[i:i + 327680][valid_mask[i:i + 327680]] = _rgbs.float()
            sigmas[i:i + 327680][valid_mask[i:i + 327680]] = sigmas_tem
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    (rays_a, xyzs, dirs,
    results['deltas'], results['ts'],results['mask'],results['rm_samples']) = \
        RayMarcher.apply(
            rays_o, rays_d, hits_t[:, 0], model.density_bitfield,kwargs['depth'],kwargs['rgb_len'],
