...
std::vector<torch::Tensor> raymarching_train(
    ...
    return raymarching_train_cu(
        rays_o, rays_d, hits_t, density_bitfield,depth,rgb_len, cascades,
...
torch::Tensor depth_loss_fw(
    ...
    return depth_loss_fw_cu(ws,ts,detals,depth, target, rays_a);
}


torch::Tensor depth_loss_bw(
    const torch::Tensor dL_dloss,
    const torch::Tensor ts,
    const torch::Tensor depth,
    const torch::Tensor target,
    ...
    return depth_loss_bw_cu(dL_dloss,  ts, depth, target, rays_a,ws,detals);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("depth_loss_fw", &depth_loss_fw);
    m.def("depth_loss_bw", &depth_loss_bw);
