...
__global__ void raymarching_train_kernel(
    ...
    uint8_t *__restrict__ density_bitfield,
    const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> depth,
    const int rgb_len,
    const int cascades,
    const int grid_size,
    const float scale,
    ...
    torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> mask)
{
    ...
    while (0 <= t && t < t2 && N_samples < max_samples)
    {
        ...
        // 步进长度指数步进初始是1/256，最小为根号三/1024，最大为根号三*grid_size_each
    ...
    const int max_samples)
{
    ...
    auto mask = torch::zeros({N_rays * max_samples}, rays_o.options());
    ...
    { raymarching_train_kernel<<<blocks, threads>>>(
        ...
        density_bitfield.data_ptr<uint8_t>(),
        depth.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        rgb_len,
        cascades,
        grid_size,
        scale,
        ...
        mask.packed_accessor32<float, 1, torch::RestrictPtrTraits>()
        ); }));
    return {rays_a, xyzs, dirs, deltas, ts,mask,  counter};
}
